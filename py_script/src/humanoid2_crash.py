...
def allJointControl_2(move_groups):
    ...
    planning_groups = [move_groups[0], 
                        move_groups[5], move_groups[4], move_groups[2],
                        move_groups[1], move_groups[6]
                        ]
    for move_gr in planning_groups:
...
# 2nd pose
    #Left Leg
    setJointValue(move_groups[2], [0.17, 0.00])
    #Left Leg
    setJointValue(move_groups[6], [-0.25])
    setJointValue(move_groups[5], [0.25, 0.0])
    planning_groups = [move_groups[2],
                       move_groups[5], move_groups[6]]
    for move_gr in planning_groups:
#        move_gr.set_goal_tolerance(0.003)
        move_gr.set_num_planning_attempts(2)
...
def external_force_to(link, x, y, z):
#    rospy.wait_for_service('/gazebo_apply_body_wrench')
    ...
    wrench.force.y = y
    wrench.force.z = z
#    force(body_name = "humanoid_shp::base", reference_point=None, wrench=wrench,start_time=rospy.Time.from_sec(5), duration = rospy.Duration.from_sec(1.0))
    force(body_name = link, reference_point=None, wrench=wrench,start_time=rospy.Time.from_sec(5), duration = rospy.Duration.from_sec(1.0))
...
#        display_trajectory_publisher = displayTrajectoryPublisher()
        ...
        if cmd_pose == '1':
            external_force_to('humanoid_shp::base', 500, 0, 0)
        elif cmd_pose == '2':
            external_force_to('humanoid_shp::base', -500, 0, 0)
        elif cmd_pose == '3':
            external_force_to('humanoid_shp::base', 0, 85, 0)
        elif cmd_pose == '4':
            external_force_to('humanoid_shp::base', 0, -85, 0)
        elif cmd_pose == '5':
            external_force_to('humanoid_shp::base', 400, 130, 0)
        elif cmd_pose == '6':
            external_force_to('humanoid_shp::base', 400, -130, 0)
        elif cmd_pose == '7':
            external_force_to('humanoid_shp::base', -400, 130, 0)
            ...
            print "\nthere a 5 different poses 1 to 6."
            print "try any number 1, 2, 3, ..., or 6."
